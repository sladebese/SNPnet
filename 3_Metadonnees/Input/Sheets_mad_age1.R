...
##A voir
#Prepare the dfmad file 
dfmad <- read.table("~/workspace/AnalysisSteph/2_Phenotypes/Output/mad_age1.csv", header = TRUE, sep = ",", row.names = 1)
rownames( dfmad) <- dfmad[ , "strain_number"]
dfmad <- dfmad[ , -which( names( dfmad) == "strain_number")]
...
#In the sheet, we take only data of interest then we merge the column with dfmad
#In this case, it permits to have correlation with same strain
  for( col_name in data_list[[ slide_name]]){
    ...
    common_strain <- current_data_df$strain_number[ current_data_df$strain_number %in% rownames( dfmad)]
    ...
    correlation <- lapply( names(dfmad), function(namePheno) {
        mg_info_df <- data.frame( pheno = dfmad[, namePheno], meta = current_data_df[ rownames( dfmad), col_name] )
    ...
    cor_list <- do.call("rbind.data.frame", 
                        lapply( names(dfmad), function(namePheno) {
                        number <-  which( names(dfmad) == namePheno)
                        mg_info_df <- data.frame( pheno = dfmad[, namePheno], meta = current_data_df[ rownames( dfmad), col_name] )
                        mg_info_df_sub <- mg_info_df[ complete.cases( mg_info_df),]
