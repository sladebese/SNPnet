...
#Prepare the diffmedian file 
diffmedian <- read.table("~/workspace/AnalysisSteph/2_Phenotypes/Output/diffmedian.csv", header = TRUE, sep = ",", row.names = 1)
rownames( diffmedian) <- diffmedian[ , "strain_number"]
diffmedian <- diffmedian[ , -which( names( diffmedian) == "strain_number")]
...
#In the sheet, we take only data of interest then we merge the column with diffmedian
#In this case, it permits to have correlation with same strain
  for( col_name in data_list[[ slide_name]]){
    ...
    common_strain <- current_data_df$strain_number[ current_data_df$strain_number %in% rownames( diffmedian)]
    ...
    correlation <- lapply( names(diffmedian), function(namePheno) {
        mg_info_df <- data.frame( pheno = diffmedian[, namePheno], meta = current_data_df[ rownames( diffmedian), col_name] )
    ...
    cor_list <- do.call("rbind.data.frame", 
                        lapply( names(diffmedian), function(namePheno) {
                        number <-  which( names(diffmedian) == namePheno)
                        mg_info_df <- data.frame( pheno = diffmedian[, namePheno], meta = current_data_df[ rownames( diffmedian), col_name] )
                        mg_info_df_sub <- mg_info_df[ complete.cases( mg_info_df),]
