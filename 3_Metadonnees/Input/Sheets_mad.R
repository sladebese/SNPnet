...
#Prepare the diffmad file 
diffmad <- read.table("~/workspace/AnalysisSteph/2_Phenotypes/Output/diffmad.csv", header = TRUE, sep = ",", row.names = 1)
rownames( diffmad) <- diffmad[ , "strain_number"]
diffmad <- diffmad[ , -which( names( diffmad) == "strain_number")]
...
#In the sheet, we take only data of interest then we merge the column with diffmad
#In this case, it permits to have correlation with same strain
  for( col_name in data_list[[ slide_name]]){
    ...
    common_strain <- current_data_df$strain_number[ current_data_df$strain_number %in% rownames( diffmad)]
    ...
    correlation <- lapply( names(diffmad), function(namePheno) {
        mg_info_df <- data.frame( pheno = diffmad[, namePheno], meta = current_data_df[ rownames( diffmad), col_name] )
    ...
    cor_list <- do.call("rbind.data.frame", 
                        lapply( names(diffmad), function(namePheno) {
                        number <-  which( names(diffmad) == namePheno)
                        mg_info_df <- data.frame( pheno = diffmad[, namePheno], meta = current_data_df[ rownames( diffmad), col_name] )
                        mg_info_df_sub <- mg_info_df[ complete.cases( mg_info_df),]
