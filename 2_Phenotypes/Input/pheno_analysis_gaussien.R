...
for (namePheno in phenotypes){
  ...
  if ( RemainingStrain_nb > 80){
    ...
    outliersMed1inf <- dfmedian_bis[is_outlier_inf( dfmedian_bis[dfmedian_bis$age ==1,]$median) & dfmedian_bis$age ==1, "strain_number"]
    outliersMed1sup <- dfmedian_bis[is_outlier_sup( dfmedian_bis[dfmedian_bis$age ==1,]$median) & dfmedian_bis$age ==1, "strain_number"]
    nameOutliersMed1 <- c(nameOutliersMed1, as.character(outliersMed1inf), as.character(outliersMed1sup))
    outliersMed4inf <- dfmedian_bis[is_outlier_inf( dfmedian_bis[dfmedian_bis$age ==4,]$median) & dfmedian_bis$age ==4, "strain_number"]
    outliersMed4sup <- dfmedian_bis[is_outlier_sup( dfmedian_bis[dfmedian_bis$age ==4,]$median) & dfmedian_bis$age ==4, "strain_number"]
    ...
    #Names of all inferior and superior extrems strains are collected for the current phenotype
    #All names are collected in a vector nameOutliers for the table of extrem strains
    outliersIMinf <- dfmedian[ is_outlier_inf( dfmedian$diff), "strain_number"]
    outliersIMsup <- dfmedian[ is_outlier_sup( dfmedian$diff), "strain_number"]
    ...
    if ( wilcoxon2$p.value < 0.05){
      if ( sum( dfmedian$sign == "positive") > sum( dfmedian$sign == "negative")) {
        action2 <- "augmenter"
        contresign <- "negative"
      } else {
        action2 <- "diminuer"
      ...
      lines_contresign <- sum( dfmedian$sign == contresign) 
      percent_contresign <- round( lines_contresign / length( dfmedian$sign), 2) * 100 
      if ( identical( action, action2)){
        cat ( "<br> Il existe une différence significative d'après le test de Wilcoxon. La valeur phénotypique de", namePheno,
             ...
             lines_contresign, "lignées ont un comportement inverse minoritaire, soit", percent_contresign, "%.")
      } else {
        cat ( "<br> Il existe une différence significative d'après le test de Wilcoxon. La valeur phénotypique de", namePheno,
    ...
    #Shapiro test pour tester la normalité de la différence des medianes 
    cat("<br>La normalité de la distribution de la différence median4 - median1 est évaluée grâce à un shapiro test. 
        Si la p-value obtenue est inférieure à 0.05, on considère que l'échantillon a une distribution non gaussienne. <br>")
    shapiro <- shapiro.test( dfmedian$diff)
    print( shapiro)
    
    #On reporte la normalité dans un tableau
    if (shapiro$p.value > 0.05){
      dfgaussien[ namePheno, "mediane_avec_extremes"] <- "Normale"
    } else {
      dfgaussien[ namePheno, "mediane_avec_extremes"] <- ""
    }
    
    #On veut faire pareil mais sans les valeurs extremes 
    dfmedian_no_outliers <- dfmedian[ -which( is_outlier( dfmedian$diff)),]
    # Plots of mediane 4 - mediane 1 for each strains, organized by increasing value
    # Same values are plotted in form of qplot and boxplot
    cat("<br>Les graphiques suivants sont semblables aux précédents, les données 'extrêmes' ont toutefois été enlevées.<br>")
    qplotmedianno <- qplot(data = dfmedian_no_outliers, x= strain_number, y = diff, col = sign) +
                    scale_fill_manual(values = c("positive" = "deepskyblue", "negative" = "red2")) +
                    geom_hline( aes( yintercept = 0), lty = 2)+
                    xlab( "Strains") +
                    ylab( "median4 - median1") +
                    theme( axis.text.x=element_blank(), legend.position ="bottom") 
    qplotmedianboxno <- ggplot( dfmedian_no_outliers) + 
                    geom_boxplot( aes( x = "strains", y = diff), fill = "gray82") 
    titlemedno2 <- paste( namePheno, "\ndifference mediane with no extrems age 4 - 1 for each strain")
    grid.arrange( qplotmedianboxno, qplotmedianno, ncol = 2, top = titlemedno2)
    #Shapiro test pour tester la normalité de la différence des medianes 
    cat("<br>La normalité de la distribution de la différence median4 - median1 (sans extrêmes) est évaluée grâce à un shapiro test. 
        Si la p-value obtenue est inférieure à 0.05, on considère que l'échantillon a une distribution non gaussienne. <br>")
    shapirono <- shapiro.test( dfmedian$diff)
    print( shapirono)
    
    #On reporte la normalité dans un tableau
    if (shapirono$p.value > 0.05){
      dfgaussien[ namePheno, "mediane_sans_extremes"] <- "Normale"
    } else {
      dfgaussien[ namePheno, "mediane_sans_extremes"] <- ""
    }
    ...
    #Shapiro test pour tester la normalité de la différence des mad 
    cat("<br>La normalité de la distribution de la différence mad4 - mad1 est évaluée grâce à un shapiro test. 
        Si la p-value obtenue est inférieure à 0.05, on considère que l'échantillon a une distribution non gaussienne. <br>")
    ...
    if (shapiro2$p.value > 0.05){
      dfgaussien[ namePheno, "mad_avec_extremes"] <- "Normale"
    } else {
      dfgaussien[ namePheno, "mad_avec_extremes"] <- ""
    }
    
    #On veut faire pareil mais sans les valeurs extremes 
    dfmad_no_outliers <- dfmad[ -which( is_outlier( dfmad$diff)),]
    # Plots of mad 4 - mad 1 for each strains, organized by increasing value
    # Same values are plotted in form of qplot and boxplot
    cat("<br>Les graphiques suivants sont semblables aux précédents, les données 'extrêmes' ont toutefois été enlevées.<br>")
    qplotmadno <- qplot(data = dfmad_no_outliers, x= strain_number, y = diff, col = sign) +
                  scale_fill_manual(values = c("positive" = "deepskyblue", "negative" = "red2")) +
                  geom_hline( aes( yintercept = 0), lty = 2)+
                  xlab( "Strains") +
                  ylab( "mad4 - mad1") +
                  theme( axis.text.x = element_blank(), legend.position = "bottom") 
    qplotmadboxno <- ggplot( dfmad_no_outliers) + 
                  geom_boxplot( aes( x = "strains", y = diff), fill = "gray82")
    titlemadno2 <- paste( namePheno, "\ndifference mad with no extrems age 4 - 1 for each strain")
    grid.arrange( qplotmadboxno, qplotmadno, ncol = 2, top = titlemadno2)
    
    #Shapiro test pour tester la normalité de la différence des mad 
    cat("<br>La normalité de la distribution de la différence mad4 - mad1 (sans extrêmes) est évaluée grâce à un shapiro test. 
        Si la p-value obtenue est inférieure à 0.05, on considère que l'échantillon a une distribution non gaussienne. <br>")
    shapirono2 <- shapiro.test( dfmad$diff)
    print( shapirono2)
    
    #On reporte la normalité dans un tableau
    if (shapirono2$p.value > 0.05){
      dfgaussien[ namePheno, "mad_sans_extremes"] <- "Normale"
    } else {
      dfgaussien[ namePheno, "mad_sans_extremes"] <- ""
    }
    ...
    mgmedmad <- merge.data.frame( x = dfmedian_no_outliers, y = dfmad_no_outliers, by = "strain_number" )
...
#      mad_python, median_python, shapiro, shapiro2)
