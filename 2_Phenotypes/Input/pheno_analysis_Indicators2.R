...
for ( namePheno in phenotypes){
  ...
  if ( RemainingStrain_nb > 80){
    ...
    dfIM$diff <- (2* ( dfIM$median4 - dfIM$median1)) / ( dfIM$median4 + dfIM$median1)
    ...
    if ( sum( is_outlier( dfIM$diff) != 0)){
    qplotIMbox <- qplotIMbox +
                      geom_text_repel( 
    ...
    dfIV <- merge.data.frame( x = dfmad[ , c( "strain_number", "mad1", "mad4")], 
                              ...
                              by = "strain_number")
    dfIV$diff <- ( 2* (( dfIV$mad4/dfIV$median4) - ( dfIV$mad1/dfIV$median1)))/ (( dfIV$mad4/dfIV$median4) + ( dfIV$mad1/dfIV$median1))
    ...
    if ( sum( is_outlier( dfmad_bis[ dfmad_bis$age ==1, "mad"])) != 0) {
      plotpheno14mad <- plotpheno14mad +
                        geom_text_repel( 
    ...
    if ( sum( is_outlier( dfmad_bis[ dfmad_bis$age ==4, "mad"])) != 0) {
      plotpheno14mad <- plotpheno14mad +
                        geom_text_repel( 
    ...
    qplotIVbox <- ggplot( dfIV) + 
                  ...
                  ylab( "IV") +
                  theme( axis.text.x = element_blank())
    if ( sum( is_outlier( dfIV$diff) != 0)){
      qplotIVbox <- qplotIVbox +
...
#      dfIM_bis, outliersMed1, outliersMed4, plotpheno14med, wilcoxon2, qplotmedian,
#      qplotmedianbox, qplotmedian14, spearman, dfmad, strainordermad, dfmad_bis, outliersMad1,
#      outliersMad4, plotpheno14mad, qplotmad, qplotmadbox, qplotmad14, spearman2, mgIMIV,
