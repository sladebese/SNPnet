...
##Objective : Tables and heatmap foloowing the analysis of phenotypes 
...
cat( "La table suivante est un récapitulatif des distributions qui sont normales ou non, 
     détaillées plus haut pour chaque phénotypes")

kable( dfgaussien, caption = "**Normality of distributions** : normal distribution are indicated in the table")
...
cat( "La table qui suit montre les lignées qui ont des comportements extrêmes au moins deux fois dans le cas
    de la médiane ou de la mad, ce pour chaque âge. Ce tableau ne parle pas des différences de médiane et de mad, 
    qui seraient tout aussi intéressantes à répertorier, mais pour le GWAS.<br>")
...
datatable( mgmedmad14)

cat("<H5> Heatmap of median extrem strains </H5><br>")

#Same data in another format
cat ( "<br>Les heatmap suivantes montrent les données du tableau sous un autre format, 
     si les lignées étaient considérées comme extrêmes pour le phénotype x à la médiane âge 1
     par exemple, on les retrouve en couleur sur le premier graphique. Les données ont ensuite été
     clusterisées.")
...
cat("Les deux heatmap suivante représentent lindicateur de moyenne pour les phenotypes étudiés. 
...
#Reorganize 
  ...
  rownames( diffmedian) <- diffmedian[ , 1]
  diffmedian <- diffmedian[ , -1]
  ...
  diffmedian_sub <- diffmedian[ complete.cases( diffmedian),]
  ...
  diffmedian_sub2 <- diffmedian[ , -which( names( diffmedian) %in% nameDrop)]
  #Create correlation matrix
  ...
  cat("<br><br><br>On fait de même avec l'indicateur de variation.<br>")
  
  #Reorganize 
  #First column as rownames
  rownames( diffmad) <- diffmad[ , 1]
  diffmad <- diffmad[ , -1]
  
  cat("Première proposition : On enlève les lignes avec des données manquantes, on traite donc les 29 phénotypes
      avec 138 observations.<br>")
  #Treating only complete rows 
  diffmad_sub <- diffmad[ complete.cases( diffmad),]
  #Create correlation matrix
  corrmad_sub <- round( cor( diffmad_sub, method = "spearman"), 2)
  #Reorder correlation 
  ...
  cat("<br>Deuxième proposition : Les phénotypes avec des données manquantes sont enlevés, on traite donc 26 phenotypes
      mais avec les 163 observations.<br>")
  diffmad_sub2 <- diffmad[ , -which( names( diffmad) %in% nameDrop)]
  #Create correlation matrix
  corrmad_sub2 <- round( cor( diffmad_sub2, method = "spearman"), 2)
  #Reorder correlation 
